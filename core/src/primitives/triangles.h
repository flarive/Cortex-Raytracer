...
public:
    ...
    TriangleHitable(const std::string& file_name, shared_ptr<material> _material) : mat(_material)
    {
    ...
    ~TriangleHitable()
    {
        deconstruct_bvh_tree(this->bvh_root);
    }

    void print_bvh(TriangleBVH* bvh, int depth = 0)
    {
        if (bvh->num_inds) {
            std::clog << std::setw(depth) << "\tnum_indices = " << bvh->num_inds << ", first = " << bvh->inds[0] << ", last = " << bvh->inds[bvh->num_inds - 1] << std::endl;
        }
        else {
            std::clog << std::setw(depth) << "\tNon-leaf" << std::endl;
        }
    ...
    void deconstruct_bvh_tree(TriangleBVH* node)
    {
        if (node->num_inds) {
            delete[] node->inds;
        }
        ...
        if (node->l) {
            deconstruct_bvh_tree(node->l);
        }

        if (node->r) {
            deconstruct_bvh_tree(node->r);
        }
    ...
    void compute_separating_boxes(const std::vector<std::pair<float, int>>& vals, aabb& b0, aabb& b1)
    {
