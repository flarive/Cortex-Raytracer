...
void MultiJittered::generate_samples()
{
    ...
    for (int p = 0; p < num_sets; p++) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                samples[i * n + j + p * num_samples].x() = (i * n + j) * subcell_width + randomizer::random_float(0, subcell_width);
                samples[i * n + j + p * num_samples].y() = (j * n + i) * subcell_width + randomizer::random_float(0, subcell_width);
            }
        }
    }
    ...
    for (int p = 0; p < num_sets; p++) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                int k = randomizer::random_int(j, n - 1);
