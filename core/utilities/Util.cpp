...
double _atan2_approx_private(const double y, const double x) noexcept {
    ...
    constexpr auto n1 = static_cast<double>( 0.97239411);
    constexpr auto n2 = static_cast<double>(-0.19194795);
...
double _asin_approx_private(const double x) noexcept {
    ...
    constexpr double lim = 0.8;
    if ((x < static_cast<double>(-lim)) || (static_cast<double>(lim) < x))
        return std::asin(x);

    // The taylor series approximation
    // TODO [correctess] Could we add another layer of depth without a performance hit?
    constexpr auto a = static_cast<double>(0.5);
    constexpr auto b = static_cast<double>(a * 0.75);
    constexpr auto c = static_cast<double>(b * (5.0 / 6.0));
    constexpr auto d = static_cast<double>(c * (7.0 / 8.0));

    const auto aa = static_cast<double>((x * x * x) / 3.0);
    const auto bb = static_cast<double>((x * x * x * x * x) / 5.0);
    const auto cc = static_cast<double>((x * x * x * x * x * x * x) / 7.0);
    const auto dd = static_cast<double>((x * x * x * x * x * x * x * x * x) / 9.0);
...
inline double _cos_73s(const double x) noexcept {
    // TODO [correctness] See if the ones with more precision are just as efficient
    constexpr auto c1 = static_cast<double>( 0.999999953464);
    constexpr auto c2 = static_cast<double>(-0.499999053455);
    constexpr auto c3 = static_cast<double>( 0.0416635846769);
    constexpr auto c4 = static_cast<double>(-0.0013853704264);
    constexpr auto c5 = static_cast<double>( 0.00002315393167);
...
double _cos_approx_private(double x) noexcept {
    ...
    constexpr auto a = static_cast<double>(-1.0 / ( 2.0));
    constexpr auto b = static_cast<double>( 1.0 / ( 4.0 *  3.0 *  2.0));
    constexpr auto c = static_cast<double>(-1.0 / ( 6.0 *  5.0 *  4.0 *  3.0 *  2.0));
    constexpr auto d = static_cast<double>( 1.0 / ( 8.0 *  7.0 *  6.0 *  5.0 *  4.0 * 3.0 * 2.0));
    constexpr auto e = static_cast<double>(-1.0 / (10.0 *  9.0 *  8.0 *  7.0 *  6.0 * 5.0 * 4.0 * 3.0 * 2.0));
    constexpr auto f = static_cast<double>( 1.0 / (12.0 * 11.0 * 10.0 *  9.0 *  8.0 * 7.0 * 6.0 * 5.0 * 4.0 * 3.0 * 2.0));
    constexpr auto g = static_cast<double>(-1.0 / (14.0 * 13.0 * 12.0 * 11.0 * 10.0 * 9.0 * 8.0 * 7.0 * 6.0 * 5.0 * 4.0 * 3.0 * 2.0));
    ...
    x = fmod(x, M_2_PI);                 // Modulo it to (0, 2Pi)
    x = x * x;                          // Square x
    const double x2 = x * x;             // (and then that too)
    const double x3 = x * x * x;         // (and then that three)
    const double x4 = x * x * x * x;     // (make it four!)
